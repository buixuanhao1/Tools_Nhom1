...
public class PublicMovieController {
    ...
    public ResponseEntity<?> publicNowPlaying() {
        ...
        if (uniq.isEmpty()) {
            log.info("No showtimes found for now-playing, falling back to MovieRepository");
            java.util.List<com.movie.movie_booking_api.entity.Movie> movies = movieRepository.findByStatus(com.movie.movie_booking_api.entity.MovieStatus.NOW_SHOWING);
            for (com.movie.movie_booking_api.entity.Movie m : movies) {
                result.add(mapMovieToResponse(m, null));
            }
        } else {
            for (Long mid : uniq) {
                ...
                result.add(mapMovieToResponse(m, first));
            }
        }
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<?> publicUpcoming() {
        ...
        if (uniq.isEmpty()) {
            log.info("No showtimes found for upcoming, falling back to MovieRepository");
            java.util.List<com.movie.movie_booking_api.entity.Movie> movies = movieRepository.findByStatus(com.movie.movie_booking_api.entity.MovieStatus.UPCOMING);
            for (com.movie.movie_booking_api.entity.Movie m : movies) {
                result.add(mapMovieToResponse(m, null));
            }
        } else {
    ...
    private java.util.Map<String, Object> mapMovieToResponse(com.movie.movie_booking_api.entity.Movie m, java.time.LocalDateTime firstShowtime) {
        java.util.Map<String, Object> it = new java.util.HashMap<>();
        Long tmdbId = m.getTmdbId();
        ...
        String pp = toTmdbPath(m.getPosterUrl());
        String bp = toTmdbPath(m.getBackdropUrl());
        if (pp != null) it.put("poster_path", pp);
        if (bp != null) it.put("backdrop_path", bp);
        if (firstShowtime != null) it.put("release_date", firstShowtime.toLocalDate().toString());
        return it;
    }
